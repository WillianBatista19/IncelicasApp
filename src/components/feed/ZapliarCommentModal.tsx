...
export default function ZapliarCommentModal({
  postId,
  currentUserId,
  original,
  onSuccess,
  ...
  async function submit() {
    ...
    if (err || !data) {
      setError('Não foi possível zapliar. Tente de novo.')
      setIsPending(false)
      return
    }
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Zapliar com comentário"
        className="w-full max-w-lg rounded-2xl border border-zinc-800 bg-zinc-950 shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-zinc-800 px-5 py-4">
          <h2 className="text-base font-bold text-zinc-100">Zapliar com comentário</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Fechar"
