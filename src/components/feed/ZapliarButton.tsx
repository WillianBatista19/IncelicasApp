...
import ZapliarCommentModal from '@/components/feed/ZapliarCommentModal'
...
export default function ZapliarButton({
  postId,
  postOwnerId,
  currentUserId,
  initialRepostCount,
  ...
  // Fetch whether the current user has already zaplido this post
  useEffect(() => {
  ...
  const title = isOwnPost
    ? 'Não dá pra zapliar o próprio post'
    : !currentUserId
      ? 'Entre para zapliar'
      : active
        ? 'Você já zapliou esse post'
        : 'Zapliar'
